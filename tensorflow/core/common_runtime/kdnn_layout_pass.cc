...
// Static instance
KdnnConstStringsInfo kdnn_csinfo_;

KdnnLayoutRewritePass::KdnnLayoutRewritePass()
    : num_intra_threads_(port::MaxParallelism()) {
  // Initialize KDNN supported operations
  kdnn_csinfo_.sigmoid = "Sigmoid";
...
bool KdnnLayoutRewritePass::SigmoidRewrite(const Node* n) {
  ...
  if (IsKDNNEnabled() && !n->input_types().empty() && n->input_types()[0] == DT_FLOAT &&
